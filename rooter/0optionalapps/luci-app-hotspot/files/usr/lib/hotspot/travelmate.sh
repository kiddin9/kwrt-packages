...
f_check()
{
    ...
    do
		...
		ifname="$(ubus -S call network.wireless status | jsonfilter -l 1 -e "@.$RADIO.interfaces[@.config.mode=\"${mode}\"].device")"
		if [ -z $ifname ]; then
			break
		fi
        if [ "${mode}" = "sta" ]
...
f_scan_ap() {
	...
	trm_maxscan="10"
	scan_dev="$(ubus -S call network.wireless status 2>/dev/null | jsonfilter -q -l1 -e "@.${radio}.interfaces[0].device")"
